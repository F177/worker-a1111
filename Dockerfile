...
RUN apk add --no-cache wget && \
    # Main Model
    wget -q -O /epicrealismXL_vxviiCrystalclear.safetensors "https://huggingface.co/Fabricioi/modelorealista/resolve/main/epicrealismXL_vxviiCrystalclear.safetensors" && \
    # LoRA Model
...
# Copy models with their original names to the correct directories
COPY --from=download /epicrealismXL_vxviiCrystalclear.safetensors /stable-diffusion-webui/models/Stable-diffusion/
COPY --from=download /epicrealness.safetensors /stable-diffusion-webui/models/Lora/
