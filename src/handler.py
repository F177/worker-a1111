...
a1111_process = None
shutdown_flag = threading.Event()  # Sinaliza o desligamento do worker
...
def wait_for_service(url):
    while True:
        try:
            ...
            if r.status_code in [200, 405, 400]:  # 405/400 = endpoint existe
...
def handler(event):
    """
    Processa o job, retorna o resultado.
    """
    try:
        print("Job recebido.")
...
if _name_ == "_main_":
    try:
        ...
        # Salvar log do A1111 para depuração
        a1111_process = subprocess.Popen(
            A1111_COMMAND,
            preexec_fn=os.setsid,
            stdout=sys.stdout,  # Alternativa: open('a1111.log', 'w')
            stderr=sys.stderr
        )

        wait_for_service(f"{LOCAL_URL}/txt2img")  # Melhor que /sd-models

        print("A1111 pronto. Iniciando o handler do RunPod...")
        runpod.serverless.start({"handler": handler})

        # Espera até que o handler sinalize o encerramento
