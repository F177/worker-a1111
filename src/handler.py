...
import signal # Make sure this is imported
...
# --- A1111 Startup Configuration ---
A1111_COMMAND = [ # Re-add A1111_COMMAND
...
a1111_process = None # Re-add a1111_process variable
shutdown_flag = threading.Event() # Re-add shutdown_flag for graceful exit
...
def wait_for_service(url):
    ...
    while True:
        try:
            ...
            print("worker-a1111 - WebUI API Service is ready.") # Added print for clarity
            return
        except requests.exceptions.RequestException:
            retries += 1
            # Only log every 15 retries so the logs don't get spammed
            if retries % 15 == 0:
                print("worker-a1111 - Service not ready yet. Retrying...")
        except Exception as err:
            print(f"worker-a1111 - Error: {err}")
        time.sleep(0.2)
...
def process_images(images, job_id):
    ...
    for idx, base64_image in enumerate(images):
        try:
            filename = f"generated_image_{job_id}_{idx + 1}.png" # Made filename more unique again
            ...
            if os.environ.get("BUCKET_ENDPOINT_URL"):
                try:
                    ...
                    processed_images.append({
                        ...
                        "type": "base64",
                        "data": base64_image,
                        "error": error_msg # Include error message with fallback
                    })
            else:
            ...
            print(f"worker-a1111 - {error_msg}")
            processed_images.append({
                "filename": f"error_image_{job_id}_{idx + 1}.png", # Added job_id to error filename
                "type": "error",
                "error": error_msg
            })
...
def handler(event):
    ...
    try:
        ...
        if not images:
            ...
            # Ensure info and parameters are still passed even if no images
            return {
        ...
        errors_in_processing = [img.get("error") for img in processed_images if img.get("type") == "error" and img.get("error")]
        if errors_in_processing:
            response["errors"] = errors_in_processing
            print(f"worker-a1111 - Job {job_id} completed with errors: {errors_in_processing}")
        else:
        ...
        error_msg = f"HTTP communication error with A1111 during inference: {e}"
        print(f"worker-a1111 - {error_msg}")
        return {
            "error": error_msg,
            "job_id": job_id
...
if __name__ == "__main__":
    try:
        print("worker-a1111 - Starting A1111 server in the background...")
        # Start A1111 as a subprocess
        ...
        print(f"worker-a1111 - A fatal error occurred during startup: {e}")
    finally:
        # Ensure A1111 process is terminated on exit
        if a1111_process:
